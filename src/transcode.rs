...
//! traits rely heavily on [`Cell`]s to transport owned values across normal
//! Serde API boundaries. At a high level, each step of the transcode moves a
//! "parent" `Serializer` or `Deserializer` out of a cell, consumes it, stashes
//! away any error whose type doesn't match the method's return signature, and
//! produces an error of the correct type by generating one with a boilerplate
//! message or pulling one out of a child `Cell`.
//!
